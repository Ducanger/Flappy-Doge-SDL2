...
void doge::fall() 
{
    if (lost && pos.y < SCREEN_HEIGHT - LAND_HEIGHT - SHIBA_HEIGHT + 10)
    {
        if (time == 0)
        {
            x0 = pos.y;
...
void doge::update(short int pipeWidth, short int pipeHeight)
{
    if (!lost)
    {
        ...
        if ( (pos.x + getWidth() > posPipe[ahead].x + 3) && (pos.x + 5 < posPipe[ahead].x + pipeWidth) &&
             (pos.y + 10 < posPipe[ahead].y + pipeHeight || pos.y  + getHeight() - 5 > posPipe[ahead].y + pipeHeight + PIPE_SPACE) )
        {
            lost = true;
        }
        ...
        if ( pos.y > SCREEN_HEIGHT - LAND_HEIGHT -  SHIBA_HEIGHT + 10 || pos.y < - 10 )
        {
            lost = true;
        }
    }
